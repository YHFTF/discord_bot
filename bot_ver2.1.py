...
def 재생_로그_기록(requester: str, title: str, url: str, path="play_log.txt"):
    try:
        now = time.strftime("%Y-%m-%d %H:%M:%S")
        log_message = f"[{now}] {requester} → {title} ({url})"
        
        # 콘솔 출력
        print(f"🎵 재생 로그: {log_message}")
